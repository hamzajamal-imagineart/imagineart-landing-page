...
const PALETTES = {
  ...
  slate:   { bg: "#eef1f5", ink: "#2f4358", muted: "#97aabd", tile: "#dfe6ee", panel: "#e6ebf1", shade1: "#e3e8ee", shade2: "#d7dee6", noise: [0.4, 0.46, 0.58] },
  sage:    { bg: "#f3f6f1", ink: "#2c332b", muted: "#9dab9c", tile: "#e5ece5", panel: "#eaeee8", shade1: "#e8ede6", shade2: "#dbe3d9", noise: [0.42, 0.52, 0.4] },
  sand:    { bg: "#f4f2ed", ink: "#3a352b", muted: "#aca596", tile: "#ebe7dd", panel: "#eeebe4", shade1: "#ebe8e1", shade2: "#dfdbd1", noise: [0.56, 0.5, 0.4] },
  mineral: { bg: "#eef4f4", ink: "#283a3a", muted: "#93abab", tile: "#dee9e9", panel: "#e6eeee", shade1: "#e3ebeb", shade2: "#d6e2e2", noise: [0.38, 0.52, 0.53] },
  stone:   { bg: "#f3f2f6", ink: "#332f3a", muted: "#a39dae", tile: "#e6e4ee", panel: "#ebe9f0", shade1: "#e9e7ee", shade2: "#dcd9e4", noise: [0.48, 0.44, 0.56] },
  neutral: { bg: "#ffffff", ink: "#171717", muted: "#6e6e73", tile: "#f1f2f3", panel: "#f7f7f8", shade1: "#f1f2f3", shade2: "#e5e7ea", noise: [0.5, 0.5, 0.5] },
...
export function PageTint({ palette }: { palette: PageTintName }) {
  const { bg, ink, muted, tile, panel, shade1, shade2, noise } = PALETTES[palette];
  return (
    <style>{
      `:root{--page-bg:${bg};--ink-heading:${ink};--heading-muted:${muted};` +
      `--tile:${tile};--panel-2:${panel};--shade-1:${shade1};--shade-2:${shade2};--page-noise:${noiseUrl(noise)};}`
    }</style>
  );
}
