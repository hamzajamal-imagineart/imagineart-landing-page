...
/**
 * The connect panel on its own, so the hero can host it.
 *
 * It was a section of its own until the hero grew an MCP mode (Hamza,
 * 21 Sep); everything below the heading and lede moved across unchanged, and
 * the CSS travels with it rather than sitting in the section that no longer
 * renders. <Mcp> below still wraps it as a standalone section — pulled from
 * the page, not deleted, so putting it back is one import and one line.
 */
...
/**
 * The standalone section. Not rendered by `app/page.tsx` any more — the hero's
 * MCP chip carries the panel — but kept whole so it can go back.
 */
export function Mcp() {
  return (
