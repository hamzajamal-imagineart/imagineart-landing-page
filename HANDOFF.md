...
| Agents `#agents` | `sections/Agent` | Eyebrow, two-clause heading, then the agent recording full width with its height following the file's own ratio and nothing over it. The three-step rail under it was removed (24 Sep). |
...
| Reviews `#reviews` | `ReviewsSection` | A bento of five real Trustpilot five-star reviews (24 Sep, to a reference): three columns, a tall featured card in the middle (`hero/modes/video/2-fashion.mp4` fading into the copy, behind the review about video), dark-short over lighter-tall on the left and the mirror on the right — two shades of dark (`#141416`, `#232327`), not dark and white, since the page ships dark. Monogram avatars and "via Trustpilot" under the name, since the source gives neither photos nor titles. **The reference's stat row is left out** — no real figures; `stats` on a review renders it. Two columns under 1000px (featured across the top), one under 640. Five of the old ten quotes are no longer shown. |
...
`video` and `image` wins. **All six groups are photographs now** (Hamza,
24 Sep) — `video` still works and is unused here — four each, downscaled to 640
...
exteriors; Try On is fitting-room shots, Style Transfer motion-blur pieces
and Product one product in four scenes — those three replaced the stand-in
clips the tabs were built with. All of it is what Hamza supplied.
...
- **Written from names alone, treat as draft:** Workflows tile bodies, the four
  ...
  Try-on), the Film banner's line, the twenty-four wheel card names, and — in
...
| `use-cases/product/*.jpg` | 248KB | the Product cards, 640px; in use |
